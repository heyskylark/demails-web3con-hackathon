...
export const MessageBoxContainer = styled.div`
  width: ${({ expanded }) => (expanded ? "80vw" : "440px")};
  height: ${({ expanded }) => (expanded ? "80vh" : "468px")};
  background: #fff;
  position: fixed;
  z-index: 3;
  right: ${({ expanded }) => (expanded ? "10vw" : "67px")};
  bottom: ${({ expanded }) => (expanded ? "10vh" : "0")};
  border-top-left-radius: 10px;
  border-top-right-radius: 10px;
  box-shadow: 0 3px 8px rgba(0, 0, 0, 0.5);
  font-family: "Roboto", sans-serif;
  display: ${({ showMessage }) => (showMessage ? "none" : "flex")};
